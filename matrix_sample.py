...
import argparse
from tools.data_process import matrix_sample_time, random_split_dataset, random_split_dataset_by_matrix, split_dataset
from tools.early_stop import EarlyStopping
from tools.tool import *

parser = argparse.ArgumentParser()
parser.add_argument('--model', default='transformer_tm', help='train model name')
...
parser.add_argument('--gpu', default=1, help='use -1/0/1 chose cpu/gpu:0/gpu:1', choices=[-1, 0, 1])
parser.add_argument('--batch_size', '--bs', default=16, help='batch_size')
...
parser.add_argument('--dim_model', default=529, help='dimension of embedding vector')
parser.add_argument('--num_flows', default=529, help='dimension of embedding vector')
...
for r in range(rounds):
    ...
    # split dataset
    train_x, train_y, val_x, val_y, test_x, test_y, max_data,train_start,train_end = matrix_sample_time(data, train_rate=train_rate,val_rate=0.1,
                                                                                    seq_len=seq_len,
                                                                                    predict_len=pre_len,sample_rate=sample_rate)
    ALL_TRAIN_TIME.append([train_start,train_end])
    # ndarray -> tensor
