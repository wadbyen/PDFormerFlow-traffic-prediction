...
class PDFormerFlow(AbstractTrafficStateModel):
    """Flow-Adapted PDFormer for Traffic Matrix Modeling"""
    def __init__(self, config, data_feature):
        ...
        flow_pos_enc = self._compute_flow_positional_encoding()
        self.register_buffer('flow_pos_enc', flow_pos_enc)
    ...
    # Output layers - corrected version
        self.end_conv1 = nn.Conv2d(
            in_channels=self.skip_dim,
            out_channels=self.skip_dim,
            kernel_size=(1, self.input_window),  # Reduce temporal dimension
            bias=True,
        )
    ...
    def forward(self, batch):
        x = batch['X']  # Shape: [batch, input_window, num_flows, feature_dim]
        T = x.shape[1]
    
        # Create pattern tensors
        x_pattern_list = []
        for i in range(self.s_attn_size):
            pad_size = self.s_attn_size - 1 - i
            padded = F.pad(
                x[:, :T + i + 1 - self.s_attn_size, :, :self.output_dim],
                (0, 0, 0, 0, pad_size, 0),
                "constant", 0,
            ).unsqueeze(-2)
            x_pattern_list.append(padded)
        ...
        enc = self.enc_embed_layer(x)
        skip = 0
    
        # Process through encoder blocks
        for i, encoder_block in enumerate(self.encoder_blocks):
            enc = encoder_block(enc, x_patterns, pattern_keys, self.geo_mask, self.sem_mask)
            skip_conn = self.skip_convs[i](enc.permute(0, 3, 2, 1))  # [batch, skip_dim, num_flows, T]
            skip += skip_conn
    
        # Temporal reduction
        skip = F.relu(skip)
        skip = self.end_conv1(skip)  # [batch, skip_dim, num_flows, 1]
    
        # Feature reduction
        skip = F.relu(skip)
        skip = self.end_conv2(skip)  # [batch, output_dim, num_flows, 1]
    
        # Prepare output - FIXED
        output = skip.permute(0, 3, 2, 1)  # [batch, 1, num_flows, output_dim]
        return output
